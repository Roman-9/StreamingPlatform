...
Serial::Serial(std::string t, std::string g, std::string d,
               std::vector<int> durate, int v, std::string cp, std::string lv)
    : ContinutVideo(std::move(t), std::move(g), std::move(d), v, std::move(cp),
                    std::move(lv)),
      durateEpisoade(std::move(durate)), episodCurent(0) {}

void Serial::play() {
  if (vizioneazaEpisod()) {
    std::cout << "[BACKEND] Ai vazut episodul " << episodCurent << " din "
              << getTitlu() << "!\n";
  } else {
...
int Serial::getTimpRamas() const {
  ...
  for (size_t i = episodCurent; i < durateEpisoade.size(); ++i)
    timp += durateEpisoade[i];
  return timp;
}

int Serial::getTimpVizionat() const {
  int timp = 0;
  for (size_t i = 0; i < static_cast<size_t>(episodCurent); ++i)
    timp += durateEpisoade[i];
  return timp;
}
...
void Serial::do_afisare(std::ostream &os) const {
  os << "[SERIAL] " << getTitlu() << " (gen: " << getGen()
     << ") | Nota: " << getRating() << " | Progres: Ep. " << episodCurent
     << " din " << durateEpisoade.size() << "\n";
}
