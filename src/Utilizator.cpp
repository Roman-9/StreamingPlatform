...
Utilizator::Utilizator(std::string n, std::string p, int v)
    : nume(std::move(n)), plan(std::move(p)), varsta(v),
...
// cppcheck-suppress unusedFunction
void Utilizator::setStrategieRecomandare(
    std::shared_ptr<StrategieRecomandare> strategie) {
  if (strategie) {
...
void Utilizator::adaugaInWatchlist(const std::shared_ptr<ContinutVideo> &cv) {
  ...
  if (cv && gasesteDaca(watchlistActiva,
                        [&cv](const std::shared_ptr<ContinutVideo> &item) {
                          return item->getTitlu() == cv->getTitlu();
                        })) {
    throw std::invalid_argument("Acest titlu este deja in Watchlist!");
  }
...
void Utilizator::marcheazaCaVazut(const std::shared_ptr<ContinutVideo> &cv) {
  ...
  if (!gasesteDaca(istoricVizionari,
                   [&cv](const std::shared_ptr<ContinutVideo> &item) {
                     return item->getTitlu() == cv->getTitlu();
                   })) {
...
void Utilizator::uitaTeLaEpisod(const std::string &titluSerial) {
  auto cv =
      gasesteDaca(watchlistActiva,
                  [&titluSerial](const std::shared_ptr<ContinutVideo> &item) {
                    return item->getTitlu() == titluSerial;
                  });
  if (!cv)
    cv =
        gasesteDaca(istoricVizionari,
                    [&titluSerial](const std::shared_ptr<ContinutVideo> &item) {
                      return item->getTitlu() == titluSerial;
                    });
...
void Utilizator::acordaNota(const std::string &titlu, int nota) {
  auto cv = gasesteDaca(istoricVizionari,
                        [&titlu](const std::shared_ptr<ContinutVideo> &item) {
                          return item->getTitlu() == titlu;
                        });
...
int Utilizator::getTimpRamasWatchlist() const {
  int total = 0;
  // Parcurgem elementele din Watchlist pentru a vedea cat timp a mai ramas de
  // vizionat
...
// cppcheck-suppress unusedFunction
std::string Utilizator::getTopRecomandare(
    const std::vector<std::shared_ptr<ContinutVideo>> &catalogGlobal) const {
  if (!strategieRecomandare) {
    return "Nicio strategie de recomandare setata!";
  }

  auto topPick =
      strategieRecomandare->recomanda(catalogGlobal, istoricVizionari, varsta);
...
const ListaGenerica<ContinutVideo> &Utilizator::getWatchlist() const {
